function varargout = borders_modified(place,varargin)
...
%
% BORDERS_MODIFIED: Modified by Michelle Hirsch to support MathWorks
% Seminar "How to turn your script into a simple app".
% mhirsch@mathworks.com
%
% Modifications are to facilitate use with apps, where graphics objects
% need to be explicitly specified. Modifications are marked with a comment
% starting with MTH. Modifications are very targeted to the specific usage
% in the seminar and will likely be extremely fragile for anything else.
% 
% Modification supports this specific syntax. Anything else might break:
% TODO: Show syntax supported
...
% MTH: Remove this check to avoid call to gca without rewriting.
...
% MTH Defer this until after we've parsed input arguments
...
%                 h = patch(lon,lat,.5*[1 1 1],varargin{:}); % does not work well
         ...
         if faceplot
             ...
             %MTH: Look for explicit axes handle. It will be specified as a trailing
             %name-value pair: "Parent",ax
             if length(varargin) == 2 && strcmpi(varargin{1},'parent')
                 hax = varargin{2};
             else
                 hax = gca;
             end

             hold(hax,"on") % MTH: Specify axes
             h = plot(hax, cell2nancat(lon),cell2nancat(lat),varargin{:}); % MTH: Specify axes
         end
...
axlim = axis(hax); % make a note of the axis limits. MTH: Explicit handle. Rename variable for clarity

if ~isequal(axlim,[0 1 0 1])
   axis(hax,axlim) % MTH: Explicit handle
else
   if centerLon~=0
